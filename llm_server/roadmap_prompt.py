...
4. 주차별 할 일이 아니라 프로필, 자기소개서, 기업 정보, 채용공고를 종합해 면접에서 개인에게 할 수 있는 질문 묶음을 만드세요.
5. timeline_data의 category는 역량 키워드, 분야, 업무처럼 큰 제목으로 작성하세요.
6. subtopics의 title은 해당 큰 제목 아래의 개념 키워드처럼 짧은 소제목으로 작성하세요.
7. 각 소제목마다 questions 배열을 만들고, 질문마다 개인 맞춤 면접 질문과 답변 팁, 체크용 done 값, 꼬리질문을 작성하세요.
8. 답변 팁은 지원자 프로필·자기소개서 경험·채용공고 요구를 어떤 순서와 관점으로 연결하면 좋은지 제안하세요.
9. 학습 기준, 세부 근거, 출처 나열은 출력하지 말고 답변 방향에 필요한 힌트만 간결히 쓰세요.
10. 직무 역량 매칭도에 사용되는 strengths/gaps/required_competencies는 긴 문장이 아니라 키워드 중심으로 작성하세요.
